		...
		CapabilitiesHandler
{
	...
	public void buildTree(Instances data, boolean keepData) throws Exception
	{
		...
		//Out.println("��ӡ�������ɾ�����������");
		//Out.println(Weka.ARFF2StringWithWeight(data));
		if (keepData)
		{
			m_train = data;
		}
		...
		//Out.println("����ModelSelection��selectModel������ʵ����ʹ�õ�C45ModelSelection��selectModel����");
	...
	public StringBuffer[] toSource(String className) throws Exception
	{
		...
		else
		{
			...
			text.append("  static double N").append(
					Integer.toHexString(m_localModel.hashCode()) + printID)
					.append("(Object []i) {\n").append(
							"    double p = Double.NaN;\n");

			text.append("    if (").append(
					m_localModel.sourceExpression(-1, m_train)).append(") {\n");
			text.append("      p = ").append(
					m_localModel.distribution().maxClass(0)).append(";\n");
			text.append("    } ");
			for (int i = 0; i < m_sons.length; i++)
			{
				text.append("else if ("
