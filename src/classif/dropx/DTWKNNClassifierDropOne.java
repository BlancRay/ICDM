...
//					System.out.println("candidate null break");
					...
					for (int j = 0; j < candidatesOf.size(); j++) {
						if(tmpSequences.get(i).classValue.equals(tmpSequences.get(candidatesOf.get(j)).classValue)) {
							with++;
						}
					}
						...
						if(k != i) {
							...
							for (int l = 0; l < newNearestNeighbor.length; l++) {
								if(k != candidatesOf.get(l)) {
									double tmpD = distances[tmpSeq.index][tmpSequences.get(candidatesOf.get(l)).index];
					...
					for (int j = 0; j < newNearestNeighbor.length; j++) {
						if(tmpSequences.get(newNearestNeighbor[j]).classValue.equals(tmpSequences.get(candidatesOf.get(j)).classValue)) {
							without++;
						}
					}
