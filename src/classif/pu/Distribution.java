...
public class Distribution implements Cloneable, Serializable, RevisionHandler {
	...
	public Distribution(Instances source, ClassifierSplitModel modelToUse) throws Exception {
		...
		Instance instance;
		double[] weights;
		int nbBags=source.numClasses();

		m_perClassPerBag = new double[nbBags][0];
		m_perBag = new double[nbBags];
		totaL = 0;
		m_perClass = new double[source.numClasses()];
...
//		System.out.println(bagIndex+"   "+classIndex+"    "+m_perClassPerBag.length+"    "+m_perBag.length+"\t"+m_perClass.length);
	...
	public final double laplaceProb(int classIndex) {
		...
		return (m_perClass[classIndex] + 1) / (totaL + (double) m_perClass.length);
	}
	...
	public final double laplaceProb(int classIndex, int intIndex) {
		...
		if (Utils.gr(m_perBag[intIndex], 0))
			return (m_perClassPerBag[intIndex][classIndex] + 1.0) / (m_perBag[intIndex] + (double) m_perClass.length);
		else
