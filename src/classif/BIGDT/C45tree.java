	...
	public void prune() throws Exception {
		...
		if (!m_isLeaf) {
			...
			if (Utils.smOrEq(errorsLeaf, errorsTree + 0.1) && Utils.smOrEq(errorsLeaf, errorsLargestBranch + 0.1)) {
				...
				double[] nbObjPreClass = new double[m_train.numInstances()];
				...
				Instances splitpoint=new Instances(m_train);
				for (int j = 0; j < m_train.numInstances(); j++) {
					if (m_train.instance(j).classValue()==nbObjPreClass[classlable]) {
