...
public class Distribution implements Cloneable, Serializable
{
	...
	public void printDEBUG()
	{
		...
		//Out.println("m_perClassPerBag : ");
		//Out.println(m_perClassPerBag);
		//Out.println("m_perBag : ");
		//Out.println(m_perBag);
		//Out.println("m_perClass : ");
		//Out.println(m_perClass);
		//Out.println("totaL : ");
		//Out.println(totaL);
		//Out.println("dumpDistribution : ");
		//Out.println(dumpDistribution());
		//Out.println("==== DISTRIBUTION =======} ");
	}
	...
	/**
	 * Creates a distribution with only one bag according to instances in
	 * source.
	 * 
	 * @exception Exception
	...
	public Distribution(Distribution toMerge)
	{
		...
		System.arraycopy(toMerge.m_perClass, 0, m_perClass, 0, toMerge
				.numClasses());
		m_perClassPerBag = new double[1][0];
		m_perClassPerBag[0] = new double[toMerge.numClasses()];
		System.arraycopy(toMerge.m_perClass, 0, m_perClassPerBag[0], 0, toMerge
				.numClasses());
		m_perBag = new double[1];
		m_perBag[0] = totaL;
	}
	...
	public Distribution(Distribution toMerge, int index)
	{
		...
		m_perClassPerBag[0] = new double[toMerge.numClasses()];
		System.arraycopy(toMerge.m_perClassPerBag[index], 0,
				m_perClassPerBag[0], 0, toMerge.numClasses());
		m_perClassPerBag[1] = new double[toMerge.numClasses()];
		for (i = 0; i < toMerge.numClasses(); i++)
			m_perClassPerBag[1][i] = toMerge.m_perClass[i]
					- m_perClassPerBag[0][i];
	...
	public final int maxClass()
	{
		double p1 = (m_perClass[0] / ClassifyPOSC45.nPosSize) * ClassifyPOSC45.dDF
				* (ClassifyPOSC45.nUnSize / m_perClass[1]);
		if ((p1 > 1) || (Utils.eq(m_perClass[1], 0)))
			p1 = 1;
	 ...
	 * Returns class with highest frequency for given bag.
	 */
	public final int maxClass(int nBagIndex)
	{

		if (Utils.gr(m_perBag[nBagIndex], 0))
		{
			double p1 = (m_perClassPerBag[nBagIndex][0] / ClassifyPOSC45.nPosSize)
					* ClassifyPOSC45.dDF
					* (ClassifyPOSC45.nUnSize / m_perClassPerBag[nBagIndex][1]);
			if ((p1 > 1) || (Utils.eq(m_perClassPerBag[nBagIndex][1], 0))) //��ֹ����Ϊ0������
				p1 = 1;
	...
	/**
	 * Returns relative frequency of class over all bags with Laplace
	 * correction.
	 */
